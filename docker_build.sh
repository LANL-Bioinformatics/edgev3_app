...
build_and_save_if_missing() {
    ...
    if docker image inspect "$image_tag" >/dev/null 2>&1; then
        echo "Image $image_tag already exists locally, skipping docker buildx and docker save"
        return 0
    fi

    docker buildx build --platform "$platform" -t "$image_tag" --load -f "$dockerfile" . || \
      { echo "Docker build for $display_name failed"; 
        #exit 1; 
      }
...
docker_build() {
    # edgev3 
    build_and_save_if_missing "edgev3:20260615" "linux/$ARCH" "docker_images/Dockerfile.edgev3" "docker_images/edgev3_20260615_$ARCH.tgz" "edgev3"
    ...
    # mongodb
    build_and_save_if_missing "edgev3-mongo:20260615" "linux/$ARCH" "docker_images/Dockerfile.edgev3-mongo" "docker_images/edgev3-mongo_20260615_$ARCH.tgz" "edgev3-mongo"
